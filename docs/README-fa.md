...
Next یک نرم‌افزار (وب‌اپلیکیشن) مدیریت پروکسی است که امکان مدیریت چندصد حساب پروکسی را با سادگی و قدرت بالا فراهم می‌کند. Next از [Xray-core](https://github.com/XTLS/Xray-core) قدرت گرفته و با بک‌اند Go و داشبورد React پیاده‌سازی شده است.

## چرا Next؟

Next دارای یک رابط کاربری ساده و در عین حال پرامکانات است. Next امکان ایجاد چند نوع پروکسی برای کاربران را فراهم می‌کند بدون اینکه به تنظیمات پیچیده نیاز داشته باشید. با رابط کاربری تحت وب، می‌توانید کاربران را مانیتور، ویرایش و در صورت نیاز محدود کنید.
...
- شما لاگ های Next رو مشاهده میکنید که می‌توانید با بستن ترمینال یا فشار دادن `Ctrl+C` از آن خارج شوید
- فایل‌های Next در پوشه `/opt/next` قرار می‌گیرند
- فایل تنظیمات در مسیر `/opt/next/.env` قرار می‌گیرد ([تنظیمات](#تنظیمات) را مشاهده کنید)
- فایل‌های مهم Next در مسیر `/var/lib/next` قرار می‌گیرند
به دلایل امنیتی، داشبورد Next از طریق آی‌پی قابل دسترسی نیست. بنابراین باید برای دامنه خود گواهی SSL تهیه کنید و از طریق آدرس https://YOUR_DOMAIN:8000/dashboard/ وارد داشبورد شوید (نام دامنه خود را جایگزین YOUR_DOMAIN کنید)
- همچنین می‌توانید از فوروارد کردن پورت SSH برای دسترسی لوکال به داشبورد Next بدون دامنه استفاده کنید. نام کاربری و آیپی سرور خود را جایگزین `user@serverip` کنید و دستور زیر را اجرا کنید:

```bash
ssh -L 8000:localhost:8000 user@serverip
```

در نهایت، می‌توانید لینک زیر را در مرورگر خود وارد کنید تا به داشبورد Next دسترسی پیدا کنید:
...
برای مشاهده راهنمای اسکریپت Next دستور زیر را اجرا کنید

```bash
next --help
```

اگر مشتاق هستید که Next را از سورس و به صورت دستی اجرا کنید، مراحل زیر را مشاهده کنید
<details markdown="1">
...
در انتها، Next را به کمک دستور زیر اجرا کنید.
...
به صورت پیش‌فرض Next در آدرس `http://localhost:8000/dashboard` اجرا می‌شود. شما می‌توانید با تغییر `UVICORN_HOST` و `UVICORN_PORT` هاست و پورت را تغییر دهید.
</details>

# تنظیمات

> متغیر های زیر در فایل ‍`env` یا `.env` استفاده میشوند. شما می توانید با تعریف و تغییر آن ها، تنظیمات Next را تغییر دهید.
...
# ربات تلگرام
Next دارای یک ربات تلگرام داخلی است که می‌تواند مدیریت سرور، ایجاد و حذف کاربر و ارسال اعلان‌ها را انجام دهد. این ربات را می‌توان با چند مرحله ساده فعال کرد.
...
شما می‌توانید آدرسی را برای Next فراهم کنید تا تغییرات کاربران را به صورت اعلان برای شما ارسال کند.
...
نمونه‌ای از درخواست ارسال شده توسط Next:
...
انواع مختلف actionهایی که Next ارسال می‌کند: `user_created`, `user_updated`, `user_deleted`, `user_limited`, `user_expired`, `user_disabled`, `user_enabled`


# کمک مالی
اگر Next را برای شما مفید بوده و می‌خواهید از توسعه آن حمایت کنید، می‌توانید در یکی از شبکه های کریپتو زیر کمک مالی کنید:
...
# مشارکت در توسعه
این ❤️‍🔥 تقدیم به همه‌ی کسانی که در توسعه Next مشارکت می‌کنند! اگر می‌خواهید مشارکت داشته باشید، لطفاً [دستورالعمل‌های مشارکت](CONTRIBUTING.md) را بررسی کنید و در صورت تمایل Pull Request ارسال کنید یا یک Issue باز کنید.

لطفاً با بررسی [لیست کارها](https://github.com/aliking1367/next/issues) به ما در بهبود Next کمک کنید. کمک‌های شما با آغوش باز پذیرفته می‌شود.

<p align="center">
با تشکر از همه همکارانی که به بهبود Next کمک کردند:
</p>
<p align="center">
