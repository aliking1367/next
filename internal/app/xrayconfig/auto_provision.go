...
//   - optionally AmneziaWG, a UDP tunnel for latency-sensitive traffic
//     (see WireGuard)
//
...
// buildAutoAmneziaWG is AmneziaWG: WireGuard's data path, which is plain UDP
// with no TCP-over-TCP stalling, plus Amnezia's junk-packet handshake so the
// tunnel does not carry WireGuard's fixed, easily fingerprinted handshake
// signature. The node runs the tunnel itself and hands its traffic to Xray
// over a local tproxy inbound, so nothing here changes how the proxy inbounds
// are served.
//
// The junk parameters (jc/jmin/jmax, s1/s2, h1-h4) are left to the panel's
// own defaults in normalizeAWGSettings so a client built from these settings
// and the server always agree.
//
// MTU is 1380 rather than WireGuard's usual 1420: Amnezia's padding plus the
// extra encapsulation common on Iranian access networks otherwise pushes
// packets over the path MTU, and fragmented UDP is what makes a tunnel feel
// slow. persistent_keepalive keeps the NAT mapping alive, which prevents the
// first-packet stall after an idle moment that is very visible in games.
func buildAutoAmneziaWG(in autoProvisionBuild) (map[string]any, error) {
	privateKey, err := generateWGPrivateKey()
	if err != nil {
		return nil, err
	}
	pool := defaultAWGPoolCIDR
	serverAddress, err := tunnelServerAddress(pool)
	if err != nil {
		return nil, err
	}
	tunnelPort := derivedTunnelPort(in.port, portSet(in.usedPorts))
	if in.usedPorts != nil {
		in.usedPorts[tunnelPort] = true
	}
	return map[string]any{
		"tag":      in.tag,
		"port":     in.port,
		"protocol": AWGProtocol,
		"settings": map[string]any{
...
func portSet(ports map[int]bool) map[int]struct{} {
...
// generateWGPrivateKey returns a WireGuard private key in the standard
// base64 encoding WireGuard configs use (curve25519, clamped as the protocol
// requires).
...
// tunnelServerAddress is the tunnel-side address of the server itself: the
// first host in the pool, which is what the address allocator reserves.
func tunnelServerAddress(pool string) (string, error) {
